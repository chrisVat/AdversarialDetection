...
def get_dataset(name, batch_size):
    classes = ('adversarial', 'clean')
    transform = None

    if name == 'ciless_vit_mlp_embedding':
        trainset = EmbeddingDataset("adv_datasets/ciless_embeddings_vit_low_with_mlp_embeddings_train/mapping.csv", "adv_datasets/ciless_embeddings_vit_low_with_mlp_embeddings_train", transform=transform)
        testset = EmbeddingDataset("adv_datasets/ciless_embeddings_vit_low_with_mlp_embeddings_test/mapping.csv", "adv_datasets/ciless_embeddings_vit_low_with_mlp_embeddings_test", transform=transform)
    ...
    elif name == 'ciless_vit_v2_mlp_embedding':
        trainset = EmbeddingDataset("adv_datasets/ciless_embeddings_vit_with_mlp_embeddings_train/mapping.csv", "adv_datasets/ciless_embeddings_vit_with_mlp_embeddings_train", transform=transform)
        testset = EmbeddingDataset("adv_datasets/ciless_embeddings_vit_with_mlp_embeddings_test/mapping.csv", "adv_datasets/ciless_embeddings_vit_with_mlp_embeddings_test", transform=transform)
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=2, drop_last=True)
        testloader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=2, drop_last=True)
...
def train(epoch, max_epochs, net, trainloader, optimizer, scheduler, criterion, device, steps_per_update=1):
    ...
    steps_per_update = 1
    step = 0
    for batch_idx, (inputs, targets, alternate_images) in enumerate(trainloader):
        inputs, targets = inputs.to(device), targets.to(device)
        stacked_tensor = inputs
        for img in alternate_images:    
            stacked_tensor = torch.concat((stacked_tensor, img), 1)
        
        outputs = net(stacked_tensor)
...
def test(epoch, max_epochs, net, testloader, criterion, device):
    ...
    total = 0
    with torch.no_grad():
        for batch_idx, (inputs, targets, alternate_images) in enumerate(testloader):
            inputs, targets = inputs.to(device), targets.to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='ciless_vit_mlp_embedding', help='Dataset to train on')
